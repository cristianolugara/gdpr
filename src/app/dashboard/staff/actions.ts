...
export async function createStaffMember(formData: FormData): Promise<ActionState> {
    ...
    const rawData: Omit<StaffMember, 'id' | 'user_id' | 'created_at' | 'updated_at'> = {
        ...
        is_system_admin: formData.get('is_system_admin') === 'on',
        is_privacy_ref: formData.get('is_privacy_ref') === 'on',
        processing_allowance: formData.get('processing_allowance') as string,
